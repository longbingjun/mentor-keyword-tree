...
function pushCurve(points, width, density, birth, target, colorMode) {
  ...
  for (let i = 0; i < steps; i += 1) {
    ...
    const count = Math.max(2, Math.round(thickness * 2.1));
...
function branch(start, angle, length, depth, width, birth, target) {
  ...
  pushCurve(points, width, length * 42, birth, target, depth > 4 ? "gold" : "mixed");
...
function buildTreeData(recordsLength) {
  ...
  pushCurve([
    ...
    trunkTop
  ], trunkWidth, 130, 0.02, particles, "gold");
  ...
  for (let i = 0; i < recordsLength; i += 1) {
    const p = fruitPositions[i % fruitPositions.length];
    fruits.push({
...
export function ParticleTree({ records, litCount, selectedIndex, onFruitSelect }) {
  ...
  useEffect(() => {
    stateRef.current.litCount = litCount;
    stateRef.current.selectedIndex = selectedIndex;
    gsap.to(stateRef.current.progress, {
      value: clamp(litCount / records.length + 0.12, 0.12, 1),
      duration: 1.4,
      ease: "power3.out"
    });

    stateRef.current.fruitMeshes.forEach((mesh, index) => {
      const isCurrent = index === litCount && litCount < records.length;
      const isLit = index < litCount;
      mesh.visible = isCurrent || isLit;
      const age = Math.max(0, litCount - index);
      const targetScale = isCurrent ? 1.75 : isLit ? clamp(0.58 - age * 0.035, 0.22, 0.58) : 0.01;
      gsap.to(mesh.scale, { x: targetScale, y: targetScale, z: targetScale, duration: 0.75, ease: "back.out(1.5)" });
      gsap.to(mesh.material, { opacity: isCurrent || isLit ? 1 : 0, duration: 0.5 });
    });

    stateRef.current.memoryMeshes.forEach((mesh) => {
      mesh.visible = mesh.userData.stage < litCount;
      const target = mesh.visible ? clamp(1.08 - (litCount - mesh.userData.stage) * 0.05, 0.52, 1.08) : 0.01;
      gsap.to(mesh.scale, { x: target, y: target, z: target, duration: 0.8, ease: "power2.out" });
    });

    const final = litCount === records.length;
  ...
  }, [litCount, selectedIndex, records.length]);
  ...
  useEffect(() => {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      vertexShader: `
        ...
        void main() {
          ...
          gl_PointSize = aSize * 280.0 * uPixelRatio * appear / -mvPosition.z;
          gl_Position = projectionMatrix * mvPosition;
          vColor = color;
          vAlpha = appear * (0.2 + 0.8 * sin(aPhase + uTime * 1.2) * 0.18 + 0.74);
        }
      `,
      fragmentShader: `
    ...
    const fruitGeometry = new THREE.SphereGeometry(0.055, 24, 24);
    ...
    data.fruits.forEach((fruit, index) => {
      ...
      core.position.copy(fruit.position);
      halo.add(core);
    });
    ...
    function animate() {
      ...
      fruitGroup.children.forEach((mesh) => {
        ...
        mesh.position.set(
          base.x + Math.sin(time * 0.9 + phase) * 0.012,
          base.y + Math.cos(time * 1.1 + phase) * 0.018,
          base.z
        );
      });
      camera.position.x = lerp(camera.position.x, window.innerWidth < 860 ? 1.6 : 1.1, 0.04);
      camera.lookAt(1.8, -0.08, 0);
